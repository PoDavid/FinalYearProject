...
def build_lstm():
	...
	l_lstm = lay.LSTMLayer(l_inp, num_units=hidden_unit)
	h_layer = lay.ReshapeLayer(l_lstm, (-1, hidden_unit))
	h_layer = lay.DenseLayer(h_layer, num_units=input_len)
	l_out = lay.ReshapeLayer(h_layer, (batchsize, serie_len, input_len))
...
def test(test_fn, test_input, test_target):
	...
	# the test data is appended onto the list, so it become a three dim array, hence we extract the first one
