...
all_result_filename = 'all_res.txt'
...
def test(test_fn, test_input, test_target):
	...
	pairt.tiffer( np.array( test_target[0] ), np.array( all_res[0] ), plot_sample=0, filename=test_record_file, all_result_filename=all_result_filename )
...
def prepare_data( maindata, train_input, train_target, test_input, test_target, start_from=-1 ):
	
	global test_len, train_len, ques
	...
	if start_from == -1:
	...
	elif start_from < len( maindata )-test_len-train_len:
		# data before test cases
		while cur_pos + cut_size + 1 < start_from+train_len:	
			train_input.append( maindata[ cur_pos: (cur_pos+cut_size) ] )
			train_target.append( maindata[ cur_pos+1 : (cur_pos+cut_size+1) ] )
			cur_pos += move_size
		...
		# prepare the test cases
		test_input.append( maindata[ start_from+train_len : start_from+train_len+test_len-1 ] )
		test_target.append( maindata[ start_from+train_len+1 : start_from+train_len+test_len ] )

			
	else:
		print( 'prepare data error! start_from is not suitable.' )
	
def main():
	global apid_num, test_len, cut_size, move_size
	
	filename = 'AP_onehot1118f.npy'
	trained_path_ori = 'aprsh_t_1'
	...
	for fs in [ 10000 ]:
		...
		trained_path = trained_path_ori + '_' + str(fs)
		...
		prepare_data( maindata, train_input, train_target, test_input, test_target, start_from=fs )
		...
		for nepo in range(4):
			...
			try:
				...
				neto.store_result_tofola( trained_path, network, [ train_fn, test_fn ], [loss_record_file, test_record_file, all_result_filename] )
